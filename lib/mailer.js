...
export async function mailConfirmacion(email) {
  ...
  return getResend().emails.send({ from: fromAddress(), to: email, subject: 'Confirmá tu suscripción al seguimiento CyT', html });
}
...
export async function enviarATodos(contactos, subject, htmlFn) {
  ...
  for (const contact of contactos) {
    try {
      await getResend().emails.send({ from: fromAddress(), to: contact.email, subject, html: htmlFn(contact.email) });
      ok++;
    } catch (error) {
