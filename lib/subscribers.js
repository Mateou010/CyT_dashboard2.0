...
export async function agregarContacto(email) {
  ...
  try {
    return await getResend().contacts.create(withAudience({
      email: normalized,
      unsubscribed: false,
    }));
  } catch (error) {
    ...
    if (/already|exist|duplicate|conflict/i.test(message)) {
      return getResend().contacts.update(withAudience({
        email: normalized,
        unsubscribed: false,
      }));
    }
...
export async function listarConfirmados() {
  assertSubscribeConfig();
  const r = await getResend().contacts.list(withAudience({ limit: 100 }));
...
export async function darDeBaja(email) {
  ...
  const normalized = email.toLowerCase();
  try {
    return await getResend().contacts.update(withAudience({
      email: normalized,
      unsubscribed: true,
    }));
  } catch {
